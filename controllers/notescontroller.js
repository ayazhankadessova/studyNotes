...
const createNewNote = asyncHandler(async (req, res) => {
  ...
  if (note) {
    // Created
    return res.status(StatusCodes.CREATED).json({ message: 'New note created' })
  } else {
...
const updateNote = asyncHandler(async (req, res) => {
  ...
  if (duplicate && duplicate?._id.toString() !== id) {
    throw new BadRequestError('Duplicate note title')
  }
...
const deleteNote = asyncHandler(async (req, res) => {
  ...
  // Confirm data
  if (!id) {
    throw new BadRequestError('Note ID required')
  }
